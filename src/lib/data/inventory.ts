...
import type { AuthenticatedUser, FirestoreRecord, InventoryProductGroup, PackageData, ParsedPackageData } from "@/lib/domain/types";
...
export async function syncPackages(parsedPackages: ParsedPackageData[]): Promise<Omit<MetrcSyncResult, "storage_path">> {
  const existing = await listCollection<PackageData>(PACKAGES);
  ...
  for (const parsedPackage of parsedPackages) {
    const docId = docIdFromTag(parsedPackage.package_tag);
    seenIds.add(docId);
    const existingPackage = existingById.get(docId);

    batch.set(db.doc(`${PACKAGES}/${docId}`), {
      ...parsedPackage,
      quantity: Number(parsedPackage.quantity ?? 0),
      active: true,
      status: "active",
      last_synced_at: now(),
      updated_at: now(),
