...
import { createInteractionAction, createInteractionEntryAction } from "../../actions";
...
export default async function CompanyActivityPage({ params }: { params: Promise<{ companyId: string }> }): Promise<React.ReactElement> {
  ...
  return (
    <div>
      <PageHeader title={company.data.company_name} description={formatCompanySubheading(company.data)} />
      <CompanyTabs companySlug={companySlug} active="activity" />

      <div className="grid gap-6 xl:grid-cols-[0.85fr_1.2fr]">
        <Card>
          <CardHeader>
            <h2 className="text-base/7 font-semibold text-zinc-950 sm:text-sm/6 dark:text-white">Log Interaction</h2>
          </CardHeader>
          <CardContent>
