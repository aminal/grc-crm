...
type PackageSourceFilter = 'all' | 'in_house' | 'consignment';

type SelectionLock =
  | { kind: 'in_house' }
  | { kind: 'consignment'; distributor_id: string; distributor_name: string }
  | { kind: 'mixed' };
...
export function PackagePicker({
  ...
  const filteredPackages = useMemo(() => {
    const searched = normalizedQuery
      ? packages.filter((packageRecord) => [packageRecord.item, packageRecord.strain, packageRecord.package_tag, packageRecord.category, packageRecord.source_packages, packageRecord.consignment_distributor_name ?? ''].join(' ').toLowerCase().includes(normalizedQuery))
      : packages;
    const rows = sourceFilter === 'all' ? searched : searched.filter((packageRecord) => isConsignment(packageRecord) === (sourceFilter === 'consignment'));

    return maxVisible ? rows.slice(0, maxVisible) : rows;
  }, [maxVisible, normalizedQuery, packages, sourceFilter]);
  ...
  const selectionLock = useMemo(() => selectionLockFor(selectedPackages), [selectedPackages]);
  ...
  function isBlocked(packageRecord: PackagePickerPackage): boolean {
    if (!selectionLock || selectedTags.has(packageRecord.package_tag)) {
      return false;
    }

    if (selectionLock.kind === 'mixed') {
      return true;
    }

    if (selectionLock.kind === 'in_house') {
      return isConsignment(packageRecord);
    }

    return !isConsignment(packageRecord) || distributorKey(packageRecord) !== selectionLock.distributor_id;
  }

  function togglePackage(packageRecord: PackagePickerPackage, checked: boolean): void {
    if (checked && isBlocked(packageRecord)) {
      return;
    }
  ...
  function unitPriceLabel(packageRecord: PackagePickerPackage): string {
    ...
    return `${formatMoney(unitCents)} PPU`;
  }
              ...
              {selectedPackages.map((packageRecord) => (
                <TableRow key={packageRecord.package_tag}>
                  <TableCell>
                    ...
                    {isConsignment(packageRecord) ? (
                      <Badge color='emerald' className='mt-1.5'>Consignment
                        · {packageRecord.consignment_distributor_name}</Badge>
                    ) : null}
                  </TableCell>
  ...
  const packageDialog = (
    <Dialog size='5xl' open={dialogOpen} onClose={() => setDialogOpen(false)}>
      ...
      <DialogDescription>
        Search available METRC packages and choose the packages to include on this order.
      </DialogDescription>
      <DialogBody>
        <div className='space-y-4'>
          <div className='grid gap-3 sm:grid-cols-[1fr_12rem_auto] sm:items-end'>
            ...
            <Field label='Inventory source'>
              <Select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value as PackageSourceFilter)}>
          ...
          <div className='max-h-[60vh] space-y-3 overflow-y-auto pr-1'>
            {filteredPackages.map((packageRecord) => {
              ...
              const blocked = isBlocked(packageRecord);
              return (
                <div key={packageRecord.package_tag} className={cn(
                  'relative rounded-lg p-4 transition-colors',
                  blocked ? 'cursor-not-allowed opacity-50 bg-zinc-50 dark:bg-zinc-800' : selected ? 'cursor-pointer bg-purple-500 text-zinc-800 dark:bg-purple-500/50' : 'cursor-pointer bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700/70',
                )} onClick={() => togglePackage(packageRecord, !selected)}>
                  <Badge color={isConsignment(packageRecord) ? 'blue' : 'emerald'} className='absolute right-4 top-4'>
                    {isConsignment(packageRecord) ? 'Consignment' : 'Inventory'}
                  </Badge>
                  <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
                    <div className='flex gap-3'>
                      <Checkbox checked={selected} disabled={blocked} onClick={(event) => event.stopPropagation()} onChange={(event) => togglePackage(packageRecord, event.target.checked)} className='mt-1' />
                      <div>
                        <div className='flex flex-col'>
                          <div className='flex flex-wrap items-center font-semibold gap-2 pr-32 text-zinc-950 dark:text-white'>
                        ...
                        <div className='mt-1.5 space-y-2'>
                          <div>
                            <div className={cn(
                              ...
                              selected ? 'text-zinc-50 dark:text-black/50' : 'text-zinc-500 dark:text-white/30',
                            )}>
                              Package ID
                            </div>
                            <div className={cn(
                    ...
                    <div className='w-full lg:w-72 lg:shrink-0' onClick={(event) => event.stopPropagation()}>
                      <div
                           className={cn(
                             'text-sm font-semibold uppercase',
                             selected ? 'text-zinc-50 dark:text-black/50' : 'text-zinc-500 dark:text-white/30',
...
function isConsignment(packageRecord: PackagePickerPackage): boolean {
  return Boolean(packageRecord.consignment_distributor_id || packageRecord.consignment_distributor_name);
}

function distributorKey(packageRecord: PackagePickerPackage): string {
  return packageRecord.consignment_distributor_id || packageRecord.consignment_distributor_name || '';
}

function selectionLockFor(selectedPackages: PackagePickerPackage[]): SelectionLock | null {
  if (selectedPackages.length === 0) {
    return null;
  }

  const consignedPackages = selectedPackages.filter(isConsignment);
  if (consignedPackages.length === 0) {
    return { kind: 'in_house' };
  }

  const distributorIds = new Set(consignedPackages.map(distributorKey));
  if (consignedPackages.length !== selectedPackages.length || distributorIds.size > 1) {
    return { kind: 'mixed' };
  }

  const first = consignedPackages[0];
  return {
    kind: 'consignment',
    distributor_id: distributorKey(first),
    distributor_name: first.consignment_distributor_name || 'this distributor',
  };
}
