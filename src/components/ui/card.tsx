...
export function CardHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>): React.ReactElement {
  return <div className={cn("border-b border-zinc-950/5 dark:border-zinc-900 bg-zinc-950/45 rounded-t-lg px-5 py-4", className)} {...props} />;
}

export function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLDivElement>): React.ReactElement {
  return <h2 className={cn("text-2xl/7 font-medium text-zinc-950 sm:text-2xl/7 uppercase dark:text-white", className)} {...props} />;
}
